    ...
    <div style={{ minHeight: "100vh", background: dark ? "#03020a" : "#f0edff", color: dark ? "#f0e8ff" : "#0d0520", fontFamily: "'Outfit',sans-serif", transition: "background .4s" }}>
      ...
      <nav style={{ position: "sticky", top: 0, zIndex: 90, minHeight: 54, display: "flex", alignItems: "center", padding: "0 16px", background: dark ? "rgba(3,2,10,.9)" : "rgba(240,237,255,.9)", backdropFilter: "blur(20px)", borderBottom: "1px solid rgba(139,92,246,.13)" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 8, marginRight: "auto" }}>
          <div style={{ width: 30, height: 30, borderRadius: 9, background: "linear-gradient(135deg,#8B5CF6,#06FFF4)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 15, boxShadow: "0 3px 12px rgba(139,92,246,.4)" }}>🎓</div>
          <div>
            <span style={{ fontFamily: "'Space Mono',monospace", fontWeight: 700, fontSize: 13, color: dark ? "#f0e8ff" : "#0d0520" }}>CSE-G</span>
            <span style={{ fontFamily: "'Outfit',sans-serif", fontSize: 9, color: "rgba(139,92,246,.7)", marginLeft: 6 }}>KMIT · 2025–29</span>
          </div>
        </div>
        
        <div className="top-nav-links" style={{ display: "flex", alignItems: "center", gap: 6, marginRight: 8, overflowX: "auto", padding: "4px", background: "rgba(255,255,255,0.02)", borderRadius: 14 }}>
          {NAV.map((n) => (
            <button 
              key={n.id} 
              ...
              style={{ 
                ...
                color: page === n.id ? "#FFF" : "rgba(200,180,255,.5)", 
              ...
              onMouseOut={(e) => { if(page !== n.id) e.currentTarget.style.color = "rgba(200,180,255,.5)" }}
            >
              <span style={{ fontSize: 13, filter: page === n.id ? `drop-shadow(0 0 4px ${COLORS.v})` : "none" }}>{n.icon}</span> {n.label}
            </button>
          ))}
        </div>
